...
We now toss a needle of length $L<\ell$ onto this floor, where it lands at a uniform random position and orientation. Buffon's needle problem is to determine the probability that the needle intersects one of the lines on the floor.
...
We create a data structure `Floor` to represent the floor, characterized by the distance $\ell$ between successive parallel lines repeating ad infinitum in the $x$-direction.
